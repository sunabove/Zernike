...
def calc_moments( img, T, resolution, circle_type, device, cache=None, debug=0 ) : 
    ...
    for device_no in get_device_no_list( device ) :
        ...
        cache_img = torch.tensor( img, dtype=torch.complex128, device=cache_device )
        #cache_img = cache_img.ravel()
        cache_imgs[ device_no ] = cache_img
    pass

    moments = torch.zeros( (T + 1, 2*T + 1), dtype=torch.complex128, device=device )
    #moments = { }
    #moments[ "dimension" ] = T
...
def restore_image( moments, grid, device, cache, debug=0) :
    ...
    img = torch.zeros( (resolution, resolution), dtype=torch.complex128, device=device )
    img_rvl = img.ravel()
...
def calc_psnr( img_org, img_restored, debug=0) :
    gmax = max( numpy.max(img_org), numpy.max(img_restored) )

    if debug : print( f"gmax = {gmax}" )

    img_diff = img_org - img_restored

    mse = numpy.sum( img_diff*img_diff ) / img_diff.size

    psnr = 10*numpy.log10( gmax*gmax/mse )
...
def calc_rmse(img_org, img_restored, **options ) : 
    ...
    rmse = numpy.sqrt( numpy.sum( img_diff*img_diff ) / img_diff.size )
